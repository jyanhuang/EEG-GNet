...
file_handler = logging.FileHandler(filename='training_our.log', encoding='UTF-8', mode='w')
...
for p in range(0, 1):
   ...
   # # dataName = 'SA0' + str(p)
    ...
    data_T = sio.loadmat('data/dataset.mat')
    #data_E = sio.loadmat(datapath_E)
   # mydata_T = np.expand_dims(data_T['EEGsample'][:,27,:], axis=1)
    mydata_T = data_T['EEGsample']
  # mydata_E = data_E['data_resampled']
    Y_T = data_T['substate'].squeeze()
  #  Y_E = data_E['label'].squeeze()
    ...
    for train_index, test_index in skf.split(X, Y):
        ...
        model = net
        model.load_state_dict(torch.load('./model_save/best.pth'))
        ...
        for epoch in range(300):
            ...
            # model = net
            # model.load_state_dict(torch.load('./model_save/our20_sub0_cross5.pth'))
            model.eval()
            # net.eval()
            with torch.no_grad():
                # forward
            ...
            logger.info('test Acc = {:.5f}'.format(acc))
            filepath = './model_save/our30_sub{}_cross{}.pth'.format(p, count)
            filepat1 = './model_save/our_best_30_sub{}_cross{}.pth'.format(p, count)
            if acc > acc_bf:

                save_model = copy.deepcopy(net)
                prune.remove(save_model.conv1, 'weight')
                prune.remove(save_model.dconv3, 'weight')
                prune.remove(save_model.dconv5, 'weight')
                prune.remove(save_model.dconv6, 'weight')
                prune.remove(save_model.pconv6, 'weight')
                prune.remove(save_model.gconv2, 'theta')
                prune.remove(save_model.gconv4, 'theta')
                prune.remove(save_model.gconv7, 'theta')
                torch.save(obj=save_model.state_dict(), f=filepat1)
